...
public class WorkflowEngineImpl implements WorkflowEngine {
	...
	public WorkflowContext handle(Request request) throws InvalidRequestException, WorkflowException {
		logger.debug("Request submitted: key={}, args={}", request.getKey(), request.getArgs());
		Workflow wf = workflows.get(request.getKey());
		if (wf == null) {
			throw new InvalidRequestException("Cannot find workflow for key: " + request.getKey());
		}
		
		WorkflowContext ctx = new WorkflowContext(request.getKey(), request.getArgs());
